...
fn main() raises:
    ...
    dataraw, cols, rows = load_data(data="digits.csv")
    featuresraw, _, __ = load_data(data='digitsClasses.csv')

    X_train, X_test, y_train, y_test = train_test_split(X=dataraw, y=featuresraw, nfeatures=cols, train_split=0.945)

    #69895
    training = Matrix[1697, 64]() # HARDCODE IN HERE
    for i in range(training.rows):
        for j in range(training.cols):
            idx = i*cols + j    
            training[i,j] = X_train[idx]

    testing = Matrix[100, 64]()
    for i in range(testing.rows):
        for j in range(testing.cols):
            idx = i*cols + j    
            testing[i,j] = X_test[idx]

    trainingclasses = Matrix[1697, 1]()
    for i in range(trainingclasses.rows):
        for j in range(trainingclasses.cols):
            idx = i*1 + j    
            trainingclasses[i,j] = y_train[idx]

    testingclasses = Matrix[100, 1]()
    ...
    # SET K FOR KNN
    var K: Int = 20
    KMatrix = Matrix[20, 1]() # used for dynamic setting of variables (idk why this works)
    ...
    print("Predicted Classes for Digits:")
