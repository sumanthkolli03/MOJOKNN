...
#@parameter
#fn euclid(x1: Matrix, x2: Matrix) -> Float32:
#    var dist: Float32 = 0.0
#    for j in range(0, x1.cols, nelts):
#        var a: SIMD[type, nelts] = x1.load[nelts](0, j)
#        var b: SIMD[type, nelts] = x2.load[nelts](0, j)
#        var diff = a - b
#        var sq = diff * diff
#        dist += sq.reduce_add()  # SIMD sum
#    return dist

def euclid(x1: Matrix, x2: Matrix) -> Float32:
    var dist: Float32 = 0.0
    for j in range(x1.cols):
        dist = dist + ((x1[0, j] - x2[0, j]) ** 2.0)
    return dist


def distMat(mut training_data: Matrix, mut input_point: Matrix, mut distmat: Matrix) -> None:
    # NOTE: Needs matrix as an input, might be faster in the long run to have one matrix constantly be updated (no new pointers being made)
    # Initialize Matrices
    # distmat -> iterated on, final output of euc distances
    # matrixSlice -> iterated on, current row to have euclidean distance calculated on
    matrixSlice = Matrix[1, training_data.cols]() #POTENTIAL SPEEDUP -> PRE-INITIALIZE THIS ONCE

    for i in range(training_data.rows):
        for j in range(training_data.cols):
            matrixSlice[0, j] = training_data[i, j]
        distmat[i, 0] = euclid(matrixSlice, input_point)



fn distMatpar(training_data: Matrix, input_pointT: Matrix, mut distmat: Matrix) -> None:
...
def most_common_item(count_dict: Dict[Int, Int]) -> Float32:
...
def predict_class(training_classes: Matrix, K: Int, sorted_indices: Vector) -> Float32:
...
@always_inline
fn test_matrix_equal(A: Matrix, B: Matrix) raises -> Bool:
...
def main():
    ...
    #Pre-initializing predictedclasses to keep the same one:
    predictedclasses = Matrix[testingclasses.rows, 1]()
    testingpointT = Matrix[testing.cols, 1]()
    sorted_indices = Vector[training.rows, 1]()

    for i in range(predictedclasses.rows):

        for j in range(testing.cols):
            testingpointT[j, 0] = testing[i, j]   # might be a point of slowdown

        distmat = Matrix[training.rows, 1]()

        distMatpar(training, testingpointT, distmat) # Filling distance matrix with current testing row distances (euclidean distance, note doesn't store or sqrt -> only possible with 1 testing point)

        Sorting.simd_sort_heap(distmat, sorted_indices, distmat.rows) # SORTS DISTMAT AND RETURNS INDICES

        predictedclasses[i, 0] = predict_class(trainingclasses, K, sorted_indices) # Predicting class based on distance matrix (VOTING)
    ...
    print("Predicted Classes for MNIST:")
