...
fn main() raises:
    ...
    dataraw, cols, rows = load_data(data="cover.csv")
    featuresraw, _, __ = load_data(data='coverClasses.csv')

    X_train, X_test, y_train, y_test = train_test_split(X=dataraw, y=featuresraw, nfeatures=cols, train_split=0.9904)

  
    training = Matrix[481910, 54]() # HARDCODE IN HERE
    for i in range(training.rows):
        for j in range(training.cols):
            idx = i*cols + j    
            training[i,j] = X_train[idx]

    testing = Matrix[5000, 54]()
    for i in range(testing.rows):
        for j in range(testing.cols):
            idx = i*cols + j    
            testing[i,j] = X_test[idx]

    trainingclasses = Matrix[481910, 1]()
    for i in range(trainingclasses.rows):
        for j in range(trainingclasses.cols):
            idx = i*1 + j    
            trainingclasses[i,j] = y_train[idx]

    testingclasses = Matrix[5000, 1]()
    ...
    print("Predicted Classes for Cover:")
