...
fn main() raises:
    dataraw, cols, rows = load_data(data="cifar.csv")
    featuresraw, _, __ = load_data(data='cifarClasses.csv')

    X_train, X_test, y_train, y_test = train_test_split(X=dataraw, y=featuresraw, nfeatures=cols, train_split=0.91666)

    #250000
    training = Matrix[55000, 3072]() # HARDCODE IN HERE
    for i in range(training.rows):
        for j in range(training.cols):
            idx = i*cols + j    
            training[i,j] = X_train[idx]

    testing = Matrix[5000, 3072]()
    for i in range(testing.rows):
        for j in range(testing.cols):
            idx = i*cols + j    
            testing[i,j] = X_test[idx]

    trainingclasses = Matrix[55000, 1]()
    ...
    print("Predicted Classes for Cifar Data:")
