...
#TODO: Faster voting algorithm? Could try to avoid dictionaries
#TODO: Remove the sort from Argpartition - already gets the K smallest distances, can just vote after.
#TODO: Maybe refactor code so that we take slices/don't reallocate to a new matrix every point - might result in speedup due to caching.
...
fn runKNN(mut predictedclasses: Matrix, training: Matrix, testing: Matrix, trainingclasses: Matrix, K: Int, KMatrix: Matrix) raises:
    ...
    fn predict_one(i: Int):
        ...
        # Sort distances and get indices
            ...
            #Sorting.simd_sort_quick(local_cut_distmat, local_cut_indices, local_cut_distmat.rows)
        # Predict class
...
fn main() raises:
    ...
    dataraw, cols, rows = load_data(data="cancer.csv")
    featuresraw, _, __ = load_data(data='cancerClasses.csv')

    X_train, X_test, y_train, y_test = train_test_split(X=dataraw, y=featuresraw, nfeatures=cols, train_split=0.8)

    #69895
    training = Matrix[455, 30]() # HARDCODE IN HERE
    for i in range(training.rows):
        for j in range(training.cols):
            idx = i*cols + j    
            training[i,j] = X_train[idx]

    testing = Matrix[114, 30]()
    for i in range(testing.rows):
        for j in range(testing.cols):
            idx = i*cols + j    
            testing[i,j] = X_test[idx]

    trainingclasses = Matrix[455, 1]()
    for i in range(trainingclasses.rows):
        for j in range(trainingclasses.cols):
            idx = i*1 + j    
            trainingclasses[i,j] = y_train[idx]

    testingclasses = Matrix[114, 1]()
    ...
    # SET K FOR KNN
    var K: Int = 10
    KMatrix = Matrix[10, 1]() # used for dynamic setting of variables (idk why this works)
    ...
    print("Predicted Classes for Cancer:")
